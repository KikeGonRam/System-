```html
   ...
   <div class="container mx-auto p-6 bg-white rounded-xl shadow-lg max-w-md">
       <h1 class="text-3xl font-bold text-blue-700 mb-6 animate-title text-center">Editar Calificación</h1>
       @if ($errors->any())
       ...
       <form action="{{ route('admin.grades.update', $grade) }}" method="POST">
           ...
           <div class="mb-4">
               ...
               <select name="student_id" id="student_id" class="w-full p-2 border rounded @error('student_id') border-red-500 @enderror" required>
                   ...
                   @foreach ($students as $student)
                       <option value="{{ $student->id }}" {{ old('student_id', $grade->student_id) == $student->id ? 'selected' : '' }}>{{ $student->name }} {{ $student->lastname }}</option>
                   @endforeach
               </select>
               @error('student_id')
           ...
           <div class="mb-4">
               ...
               <select name="group_id" id="group_id" class="w-full p-2 border rounded @error('group_id') border-red-500 @enderror" required>
                   ...
                   @foreach ($groups as $group)
                       <option value="{{ $group->id }}" {{ old('group_id', $grade->group_id) == $group->id ? 'selected' : '' }}>{{ $group->name }}</option>
                   @endforeach
               </select>
               @error('group_id')
                   <span class="text-red-500 text-sm">{{ $message }}</span>
           ...
           <div class="mb-4">
               ...
               <input type="number" name="score" id="score" step="0.01" min="0" max="10" class="w-full p-2 border rounded @error('score') border-red-500 @enderror" value="{{ old('score', $grade->score) }}" required>
               @error('score')
                   <span class="text-red-500 text-sm">{{ $message }}</span>
               @enderror
           </div>
           <div class="mb-4">
               ...
               <textarea name="comments" id="comments" class="w-full p-2 border rounded @error('comments') border-red-500 @enderror" rows="4">{{ old('comments', $grade->comments) }}</textarea>
               @error('comments')
                   <span class="text-red-500 text-sm">{{ $message }}</span>
               @enderror
           </div>
           <div class="mb-6">
               ...
               <input type="date" name="date" id="date" class="w-full p-2 border rounded @error('date') border-red-500 @enderror" value="{{ old('date', $grade->date->format('Y-m-d')) }}" required>
               @error('date')
                   <span class="text-red-500 text-sm">{{ $message }}</span>
               @enderror
           </div>
           <button type="submit" class="w-full bg-blue-600 text-white font-semibold py-2 rounded-lg hover:bg-blue-700 transition duration-300">Actualizar Calificación</button>
       </form>
   </div>
   @endsection
   ```