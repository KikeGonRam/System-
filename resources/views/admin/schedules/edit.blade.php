```html
   ...
   <div class="container mx-auto p-6 bg-white rounded-xl shadow-lg max-w-md">
       <h1 class="text-3xl font-bold text-blue-700 mb-6 animate-title text-center">Editar Horario</h1>
       @if ($errors->any())
       ...
       <form action="{{ route('admin.schedules.update', $schedule) }}" method="POST">
           ...
           <div class="mb-4">
               ...
               <select name="teacher_id" id="teacher_id" class="w-full p-2 border rounded @error('teacher_id') border-red-500 @enderror" required>
                   ...
                   @foreach ($teachers as $teacher)
                       <option value="{{ $teacher->id }}" {{ old('teacher_id', $schedule->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }} {{ $teacher->lastname }}</option>
                   @endforeach
               </select>
               @error('teacher_id')
                   <span class="text-red-500 text-sm">{{ $message }}</span>
           ...
           <div class="mb-4">
               ...
               <select name="classroom_id" id="classroom_id" class="w-full p-2 border rounded @error('classroom_id') border-red-500 @enderror" required>
                   ...
                   @foreach ($classrooms as $classroom)
                       <option value="{{ $classroom->id }}" {{ old('classroom_id', $schedule->classroom_id) == $classroom->id ? 'selected' : '' }}>{{ $classroom->name }}</option>
                   @endforeach
               </select>
               @error('classroom_id')
                   <span class="text-red-500 text-sm">{{ $message }}</span>
           ...
           <div class="mb-4">
               ...
               <input type="time" name="start_time" id="start_time" class="w-full p-2 border rounded @error('start_time') border-red-500 @enderror" value="{{ old('start_time', $schedule->start_time) }}" required>
               @error('start_time')
                   <span class="text-red-500 text-sm">{{ $message }}</span>
               @enderror
           </div>
           <div class="mb-6">
               ...
               <input type="time" name="end_time" id="end_time" class="w-full p-2 border rounded @error('end_time') border-red-500 @enderror" value="{{ old('end_time', $schedule->end_time) }}" required>
               @error('end_time')
                   <span class="text-red-500 text-sm">{{ $message }}</span>
               @enderror
           </div>
           <button type="submit" class="w-full bg-blue-600 text-white font-semibold py-2 rounded-lg hover:bg-blue-700 transition duration-300">Actualizar Horario</button>
       </form>
   </div>
   @endsection
   ```